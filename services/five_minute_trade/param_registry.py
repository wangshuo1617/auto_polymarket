...
PARAM_REGISTRY: list[ParamDef] = [
    ...
    ParamDef(
        ...
        param_type="bool",
        default=False,
        description="启用早期试探入场（弱信号小仓）",
        group=GRP_DEVIATION,
        shell_var="ENABLE_EARLY_PROBE",
        sig_key="ep",
        cli_flag="enable-early-probe",
        constructor_name="enable_early_probe",
    ),
    ParamDef(
        key="early_probe_start_sec",
        param_type="float",
        default=0.0,
        description="早期试探最早生效时间（窗口内秒）",
        group=GRP_DEVIATION,
        shell_var="EARLY_PROBE_START_SEC",
        sig_key="eps",
    ),
    ParamDef(
        key="early_probe_end_sec",
        param_type="float",
        default=60.0,
        description="早期试探最晚截止时间（窗口内秒）",
        group=GRP_DEVIATION,
        shell_var="EARLY_PROBE_END_SEC",
        sig_key="epe",
    ),
    ParamDef(
        key="early_probe_min_abs_diff",
        param_type="float",
        default=10.0,
        description="早期试探触发最小BTC偏离（$）",
        group=GRP_DEVIATION,
        shell_var="EARLY_PROBE_MIN_ABS_DIFF",
        sig_key="epd",
    ),
    ParamDef(
        key="early_probe_stake_ratio",
        param_type="float",
        default=0.2,
        description="早期试探仓位比例（相对 stake_usd）",
        group=GRP_DEVIATION,
        shell_var="EARLY_PROBE_STAKE_RATIO",
        sig_key="epr",
    ),
    ParamDef(
        key="early_probe_max_entry_price",
        param_type="float",
        default=0.60,
        description="早期试探最高入场价（独立上限）",
        group=GRP_DEVIATION,
        shell_var="EARLY_PROBE_MAX_ENTRY_PRICE",
        sig_key="epp",
    ),
    ...
    ParamDef(
        key="dca_allow_pullback_add",
        param_type="bool",
        default=False,
        description="允许方向不变时回落补仓",
        group=GRP_DCA,
        shell_var="DCA_ALLOW_PULLBACK_ADD",
        sig_key="dcap",
        cli_flag="enable-dca-pullback-add",
    ),
    ParamDef(
        key="dca_pullback_ratio_min",
        param_type="float",
        default=0.03,
        description="回落补仓触发跌幅比例（相对当前持仓均价）",
        group=GRP_DCA,
        shell_var="DCA_PULLBACK_RATIO_MIN",
        sig_key="dcapr",
    ),
    ParamDef(
        key="dca_max_avg_price",
        param_type="float",
        default=0.0,
        description="DCA后均价上限（<=0 表示禁用）",
        group=GRP_DCA,
        shell_var="DCA_MAX_AVG_PRICE",
        sig_key="dcamavg",
    ),
    ...
    ParamDef(
        key="reversal_min_position_usdc",
        param_type="float",
        default=0.0,
        description="触发方向修正所需最小原仓位投入（USDC）",
        group=GRP_REVERSAL,
        shell_var="REVERSAL_MIN_POSITION_USDC",
        sig_key="drmin",
    ),
