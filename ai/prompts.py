...
RESPONSE_SCHEMA = {
    ...
    "properties": {
        "整体分析": {
            "type": "string",
            "description": "一段话概括市场环境。若有上期回顾，先简述上期判断准确度。简述当前环境对持仓是顺风还是逆风，预测未来 24h BTC 走势，以及预期月内 BTC 波动范围。"
        },
        "当前持仓与挂单分析与建议": {
            "type": "array",
...
**Part 1 - 整体分析**：一段话概括市场环境、对持仓的影响、未来 24h 与月内波动预期。若有上期回顾，先简述上期判断准确度。
...
# Core Principles (利润最大化优先)
1. **持有到期是默认策略**：对于 `position_safety_assessment` 中标记为 `safe_to_hold` 的仓位，默认建议"持有到期收割全部 Theta"，除非有极端风险信号。
2. **减仓必须量化机会成本**：任何减仓建议必须附带"放弃的 Theta 日收益"（参考 `theta_income`）。
3. **建仓/轮动以收益率为锚**：优先推荐"持有到期预期收益率"更高且安全垫更厚的标的。参考 `rotation_opportunities`。
4. **组合视角优先**：参考 `portfolio_analysis` 从组合层面评估风险，不要对单一仓位孤立恐慌。
5. **禁止复读**：若 `prediction_review` 显示上期建议未被执行，必须分析原因并给出调整后的新方案。
6. **语气校准**：对 `safe_to_hold` 仓位禁止使用恐吓措辞。只有 `at_risk` 仓位才适用紧急语气。
