...
from services.trading_lessons import format_prompt_trading_lessons, get_phase_prompt_lesson
...
def _compact_monthly_goal_context(monthly_goal_context):
    ...
    out = _select_keys(monthly_goal_context, [
        "source", "active_goal_mode", "primary_objective", "recovery_target_net_value_usdc",
        "current_net_value_usdc", "recovery_gap_to_month_start_usdc", "recovery_trade_policy",
        "recovery_mode_note",
        "target_pct", "target_pct_source", "plan_expected_return_pct",
        ...
        "total_unrealized_pnl_usdc", "total_gross_unrealized_loss_usdc",
        "total_loss_budget_used_usdc", "overall_loss_budget_usdc", "overall_loss_budget_remaining_usdc",
        "overall_loss_budget_usage_pct", "overall_loss_budget_status",
        "unrealized_loss_scope", "unrealized_loss_coverage",
    ...
    for tier in monthly_goal_context.get("tiers") or []:
        ...
        tier_out = _select_keys(tier, [
            ...
            "current_held_cost_usdc", "current_unrealized_pnl_usdc",
            "gross_unrealized_loss_usdc",
        ...
        tier_out["candidates"] = [
            _select_keys(c, [
                ...
                "held_value_usdc", "held_cost_usdc", "held_unrealized_pnl_usdc",
                "held_unrealized_loss_usdc", "pending_buy_notional_usdc", "pending_order_ids",
...
8. **必须考虑共享经验教训**：下方 `Shared Trading Lessons` 是用户复盘后的单一事实来源；分析时必须先复核 barrier 方向、是否 downside/dip、仓位集中度、离场纪律和共享经验，再看分层标签。若候选交易触发共享经验中的常犯错误，默认降级为观察/小仓 pending，除非能明确说明“本次不同”的确认信号和止损如何避免重复亏损。

{trading_lessons}

# Trading Discipline (交易纪律 - 必须遵守)
1. **月份阶段动态调整策略激进度**：
   - 月初（1-7日）：**进攻型**，但不是无差别追单。不要预设“月初没 edge”；应积极寻找“**低价 Yes + 明确催化/趋势确认**”与“**中价 No + 等待首次试探失败确认**”两类机会；禁止在月初把高价 No 追成大仓；具体月初/月末统计经验以 `Shared Trading Lessons` 为准。
...
4. **常犯错误必须反向约束仓位**：
   - 逐条复核 `Shared Trading Lessons` 中的常犯错误；若候选交易命中其中任一错误，默认降级为观察/撤单/小仓 pending。
   - 只有能明确说明“本次不同”的确认信号、失效条件、止损执行方式和金额上限，才允许例外参与；理由中必须引用对应经验教训。
...
* **本月目标**：{monthly_target}。**月度进度**参见 `收益优化上下文` 中的 `monthly_progress` 字段（含月初基准净值、当前净值、月度盈亏金额与百分比）。**分层目标与防错预算**参见 `收益优化上下文.monthly_goal_context`（各档目标仓位上限、目标盈利、已实现盈利、当前浮亏、gross realized loss + gross unrealized loss 防错预算、entry_gate、候选 token）。在**整体分析**中必须简述当前月度完成进度、主要分层余量和防错预算状态，并据此调整**机会选择**的积极性——距离目标越远且处于月初阶段，可以更积极地寻找高 Edge 机会部署闲置资金。但**月度进度或某档缺口落后绝不能成为放宽止损标准、忽略认错止损价、追价、突破目标仓位上限、或加大单笔仓位超过 Kelly 上限的理由**。保护本金永远优先于追赶目标。若 `monthly_goal_context.active_goal_mode="recovery_to_month_start"`，说明防错预算已用完，主目标必须从追求 +20% 切换为**先追回月初净值/守住月初净值**；`recovery_gap_to_month_start_usdc` 优先级高于 `total_remaining_profit_usdc`。但这不是“完全不建仓”：必须参考 `recovery_trade_policy` 主动寻找小仓、强确认、可止损的恢复型交易；只禁止扩大错误风险、近障碍接刀和全尺寸追回交易。
...
* **分层目标与防错预算检查（强制）**：必须参考 `monthly_goal_context.tiers`，识别哪些档位已经完成、哪些仍有 `remaining_profit_usdc` 缺口、哪些候选 token 符合当前阶段阈值，以及每档 `entry_gate` 是否为 allow/caution/block。防错预算已同时纳入已实现 gross 亏损和当前 gross 浮亏；`loss_budget_used_usdc` / `total_loss_budget_used_usdc` 比单看 realized 更优先。若 `active_goal_mode=recovery_to_month_start`，本轮应先输出“回到月初净值”的恢复计划：优先止损/减仓 at_risk、取消危险 buy pending、保留 safe_to_hold 的可预期 theta；同时必须主动评估是否存在 `recovery_trade_policy.allowed_trade_types` 中的小仓恢复交易。`overall_loss_budget_status=stop_new_entries` 只禁止正常尺寸/追目标建仓，不应机械禁止所有买入；若交易满足强确认、可止损、不靠近 barrier、金额不超过 `recovery_trade_policy.single_trade_cap_usdc` 且总新增风险不超过 `total_new_risk_cap_usdc`，可以输出买入或 pending buy。若没有合格恢复交易，必须说明缺少哪个确认条件，而不是只说“预算用完不买”。`target_position_gap_usdc` / `headroom_to_position_limit_usdc` 已扣除当前持仓与 active buy pending，但仍只是手动/计划仓位余量，不是必须补满的任务；新增买入只能使用 `headroom_to_risk_adjusted_cap_usdc` 与候选的 `target_position_usdc`，或在恢复模式下使用 `recovery_trade_policy` 的更小额度。若 `unattributed_pending_buy_notional_usdc > 0`，必须提示有 pending 未能归入当前分层，新增买入更保守。若手动目标高于 `phase_suggested_position_pct` 或 `effective_position_cap_pct`，必须明确说明“保留手动计划显示，但新增按更保守有效上限”。若 `entry_gate=block` 或 `overall_loss_budget_status=stop_new_entries`，默认不得新增正常买入；例外仅限符合 `recovery_trade_policy` 的小仓恢复交易、风险降低型轮动、或保护利润动作。若 `entry_gate=caution`，只能给更小仓位、更强确认、分批 pending。缺口只能用于决定“优先看哪档”，不能用于推荐不符合阶段阈值或风控纪律的 token。
...
* **三种波段策略类型**：
  ...
  2. **恐慌错配 / 试探失败修复**：当 BTC 急跌时，下方 dip 标的的 Yes 价格可能被恐慌推高（超过模型公允价），此时买入其 No 等待价格回归。判断依据：token 当前价 vs `model_yes_prob`，偏差≥20% 时有错配机会。但是否属于近障碍接刀、观察区或试探失败修复，必须按 `Shared Trading Lessons` 中的接刀/修复规则判定；不满足修复区条件时禁止买入。
...
* **本月目标对齐**：新建仓优先从 `monthly_goal_context.tiers[].remaining_profit_usdc` 缺口较大、`entry_gate` 未 block、且 `candidates` 符合当前阶段阈值的档位中选择；若某档 `realized_pnl_usdc` 已超过 `target_profit_usdc`，或防错预算进入 caution/block，默认降级为保护利润/观察，除非有明确高质量 edge 且仓位显著缩小。不得为了填补缺口而建议命中 `Shared Trading Lessons` 常犯错误的交易，或突破 `risk_budget` 的单 token 仓位/目标仓位上限。中价 No 必须逐 token 检查 `candidates[].mid_no_entry_gate` / `candidates[].entry_gate`：block 不买，caution 只能小额更低挂单，allow 也要确认 BTC 没有朝 barrier 快速移动。对 `dip to X No`，必须按共享经验中的近障碍接刀/观察区/修复区规则判定；只有满足修复区确认且能定义“再次跌回低点即退出”的情况下，才允许按恢复型小仓交易处理。
...
def _get_monthly_phase_context(day: int) -> str:
    ...
    if day <= 7:
        return (
            ...
            f"{get_phase_prompt_lesson('月初')}"
            "弹药分批动用，单次建仓不得突破 risk_budget 与 suggested_max_alloc_usdc；不允许用月初激进度替代止损纪律。"
        )
    elif day <= 22:
        return (
            f"当前为**月中（第{day}天）**，风险偏好：**平衡偏进攻型**。"
            f"{get_phase_prompt_lesson('月中')}"
        )
    else:
        return (
            f"当前为**月末（第{day}天）**，风险偏好：**防守型**。"
            f"{get_phase_prompt_lesson('月末')}"
        )
...
def get_system_instruction(
    ...
    return SYSTEM_INSTRUCTION_TEMPLATE.format(
        ...
        trading_lessons=format_prompt_trading_lessons(),
    )
