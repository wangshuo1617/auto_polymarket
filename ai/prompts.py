...
RESPONSE_SCHEMA = {
    ...
    "properties": {
        ...
        "操作清单": {
            "type": "array",
            "description": "本次所有可执行动作的扁平清单，按优先级排序。涵盖：① 已持仓的加减仓/撤单 ② 未持仓的新建仓 ③ 波段进出场。每条聚焦一个具体动作，便于人工或自动执行器直接下单。所有需要风控的进攻性/波段买入条目都必须填写 止盈目标 与 止损规则；纯撤单或仅观察类条目可省略。",
            "items": {
                "type": "object",
                "properties": {
...
**Part 1 - 整体分析**：一段话概括市场环境、对持仓的影响、月内波动预期。若有上期回顾，先简述上期判断准确度。短期价格预测细节放在 Part 1.5 中。轮动机会也在此处点名（具体买卖动作进操作清单）。
