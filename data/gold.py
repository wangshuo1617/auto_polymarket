"""
COMEX 黄金期货 (GC) 价格与 K 线数据
数据源：Yahoo Finance (yfinance) — GC=F 为 CME 黄金期货，免费无需 API Key。
结算规则：以 CME Active Month 官方 Settlement 为准，盘中价/高低点不计。
"""
...
# Yahoo Finance 标的：COMEX 黄金期货连续
GOLD_SYMBOL = "GC=F"


def get_gold_price() -> float:
    """获取 COMEX 黄金期货当前价格（美元/盎司）。"""
    if yf is None:
...
def get_gold_4h_klines_data(limit: int = 42) -> List[list]:
    """获取黄金 4h K 线数据，格式与 data.binance.get_4h_klines_data 一致。"""
    if yf is None:
        raise RuntimeError("请安装 yfinance: pip install yfinance")
    ticker = yf.Ticker(GOLD_SYMBOL)
...
def get_gold_1d_klines_data(limit: int = 30) -> List[list]:
    """获取黄金 1d K 线数据，默认近 30 天，格式与 data.binance.get_1d_klines_data 一致。"""
    if yf is None:
        raise RuntimeError("请安装 yfinance: pip install yfinance")
    ticker = yf.Ticker(GOLD_SYMBOL)
